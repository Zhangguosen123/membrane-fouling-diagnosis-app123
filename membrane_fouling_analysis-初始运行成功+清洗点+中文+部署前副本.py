...
def read_csv_robust(path, encoding_list=("utf-8-sig", "utf-8", "gbk", "latin1")):
...
def calculate_metrics(J_obs, J_pred):
    ...
    denom_range = (np.max(y) - np.min(y) + 1e-12)
    nrmse = rmse / denom_range if (np.max(y) - np.min(y)) > 0 else np.nan
...
def find_cleaning_time(t, J_pred, J0, acceptable_ratio=0.7):
    """Find time when flux drops to a ratio of initial flux (based on fitting curve)."""
    ...
    if stage == "partial":  # 100%-70% stage
        if dominant_idx == 3:
            return (
                ...
                f"Recommended strategy: Backwashing (pressure 0.08-0.1 MPa, time 3-5 min)\n"
                f"Optimization suggestion: This stage is mainly surface cake, which can be effectively restored by backwashing"
            )
        elif dominant_idx in (0, 1):
            return (
                f"Dominant fouling: {dominant_mechanism} ({dominant_ratio}%)\n"
                f"Recommended strategy: Citric acid solution soaking (concentration 1-2%, time 10-15 min) + Backwashing\n"
                f"Optimization suggestion: Early internal fouling needs timely treatment to avoid pollutants penetrating into membrane pores"
            )
        elif dominant_idx == 2:
            return (
                f"Dominant fouling: {dominant_mechanism} ({dominant_ratio}%)\n"
                f"Recommended strategy: Mild alkaline cleaning (NaOH solution, pH=9-10, time 10 min) + Backwashing\n"
                f"Optimization suggestion: Control cleaning intensity to protect membrane structure"
            )
        else:
            return (
        ...
        if dominant_idx == 3:
            return (
                f"Dominant fouling: {dominant_mechanism} ({dominant_ratio}%)\n"
                f"Recommended strategy: Backwashing (pressure 0.1 MPa, time 5 min) + Sodium hypochlorite cleaning (concentration 500 ppm, time 10 min)\n"
                f"Optimization suggestion: Appropriately reduce operating pressure to reduce cake layer compaction"
            )
        elif dominant_idx in (0, 1):
            return (
                f"Dominant fouling: {dominant_mechanism} ({dominant_ratio}%)\n"
                f"Recommended strategy: Citric acid solution soaking (concentration 2%, time 20 min) + Backwashing (pressure 0.15 MPa, time 8 min)\n"
                f"Optimization suggestion: Pretreatment to remove small molecule pollutants and reduce internal fouling risk"
            )
        elif dominant_idx == 2:
            return (
                f"Dominant fouling: {dominant_mechanism} ({dominant_ratio}%)\n"
                f"Recommended strategy: Alkaline cleaning (NaOH solution, pH=10, time 15 min) + Backwashing (pressure 0.12 MPa, time 6 min)\n"
                f"Optimization suggestion: Control feed flow rate to enhance shear force and break pore orifice bridging"
            )
        else:
...
def load_validation_data(data_type, data_id):
    """Load validation data (使用相对路径，匹配GitHub仓库中数据文件的位置)"""
    ...
    if len(J_clean) > 0:
        J0 = J_clean[0]
        if J0 <= 0:
            raise ValueError(f"数据文件 {filename} 中的初始通量为0或负数，请检查数据")
    else:
        raise ValueError(f"数据文件 {filename} 中无有效通量数据")

    return t_clean, J_clean, J0, filename


# ==============================================================================
# <<-- Analysis Logic for Uploaded Files
# ==============================================================================
def analyze_uploaded_file(uploaded_file):
    """Analyze user-uploaded CSV file"""
    try:
        # 读取上传的文件
        ...
        if "Time (s)" not in df.columns or "Flux" not in df.columns:
            raise ValueError("上传的文件必须包含时间（Time (s)）和通量（Flux）列，请检查数据格式。")
        ...
        if len(J_clean) < 5:
            raise ValueError("有效数据点不足，无法进行拟合分析。")

        J0 = J_clean[0]
        if J0 <= 0:
            raise ValueError("初始通量为0或负数，请检查数据。")

        # 1. 全流程分析
        params_full = fit_model(t_clean, J_clean, J0)
        J_pred_full = stage1_model(params_full, t_clean - t_clean[0], J0)
        metrics_full = calculate_metrics(J_clean, J_pred_full)
        eta_full = calculate_mechanism_contribution(params_full, t_clean, J0)

        # 2. 找到70%清洗点
        cleaning_time_sec, cleaning_flux, cleaning_idx = find_cleaning_time(t_clean, J_pred_full, J0, 0.7)

        # 3. 100%-70%阶段分析
        t_partial = t_clean[: cleaning_idx + 1]
...
def analyze_single_file(data_type, data_id):
    ...
    try:
        ...
        cleaning_time_sec, cleaning_flux, cleaning_idx = find_cleaning_time(t_clean_sec, J_pred_full, J0, 0.7)
...
def main():
    ...
    analysis_mode = st.sidebar.selectbox(
        ...
        ("Single File Analysis", "Batch Analysis of All Files", "Upload & Analyze Custom File"),
    )

    all_results = []

    if analysis_mode == "Single File Analysis":
        st.header("📊 Single File Analysis (Preset Data)")
    ...
    elif analysis_mode == "Batch Analysis of All Files":
        st.header("📊 Batch Analysis of All Preset Files")
    ...
    elif analysis_mode == "Upload & Analyze Custom File":
        st.header("📤 Upload & Analyze Custom Data File")
        st.info("📌 Please upload a CSV file containing time and flux data (columns will be auto-detected).")
            ...
            if res["success"]:
                with st.expander(f"📄 Detailed Report for {res['filename']}", expanded=False):
                    ...
                    with col1:
                        ...
                        filtered_sizes_full, filtered_labels_full = [], []
                    ...
                    with col2:
                        ...
                        filtered_sizes_partial, filtered_labels_partial = [], []
                    ...
                    ax3.plot(
                        ...
                        "-",
                        lw=2,
                        label="Full Process Fitting Curve",
                        color="orange",
                        alpha=0.8,
                    )
                    ...
                    ax3.plot(
                        ...
                        "-",
                        lw=3,
                        label="100%-70% Stage Fitting Curve",
                        color="green",
                    )
